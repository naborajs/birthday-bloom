...
| `VITE_LANGUAGE` | string / enum | `en` | Multi-language localization switch. Options: `en` (English - default), `hi` (Hindi), `bn` (Bengali), `fr` (French). Normalizes `fr`/`french`/`francais`, `bn`/`bengali`/`bangla`, and `hi`/`hindi`/`in` aliases. Also accepted as `VITE_LANG`. Includes automatic fallback to English if omitted or invalid. See [[setup-french|French Guide]], [[setup-hindi|Hindi Guide]], and [[setup-bengali|Bengali Guide]]. |
